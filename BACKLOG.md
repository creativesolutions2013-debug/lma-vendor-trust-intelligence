# MVP Backlog

## MVP 1 — Foundation

- [x] Vendor inventory
- [x] Engagement/service model
- [x] Intake workflow
- [x] Inherent risk calculator
- [x] Residual risk calculator
- [x] Vendor 360
- [x] Control Tower attention queue
- [x] Findings tracking
- [x] Monitoring event entry
- [x] Risk register export

## MVP 2 — Assessment & evidence

- [x] Assessment model
- [x] Risk-based assessment recommendation
- [x] Assessment creation and assignment
- [x] Assessment status / approval tracking
- [x] Evidence Center
- [x] Evidence upload
- [x] SOC 2 / ISO / pentest / PCI metadata capture
- [x] Evidence expiration tracking
- [x] Evidence visibility in Vendor 360
- [x] Automated SOC 2 metadata extraction
- [ ] Automated pentest metadata extraction

## MVP 3 — Assurance intelligence

- [ ] Common control framework
- [ ] Framework mappings
- [x] AI evidence analyst (SOC 2 first iteration)
- [ ] Evidence confidence scoring
- [ ] Contradiction detection
- [ ] AI risk statement drafting

## MVP 4 — Continuous monitoring

- [ ] CISA KEV ingestion
- [ ] NVD/CVE ingestion
- [ ] Security rating import
- [ ] Event policy engine
- [ ] Triggered reassessment
- [ ] Risk escalation workflow

## Production hardening

- [ ] PostgreSQL persistence
- [ ] Object storage for evidence
- [ ] Multi-tenant RBAC
- [ ] Audit log
- [ ] Encryption / secret management
- [ ] File malware scanning

## Later integrations

- [ ] BitSight
- [ ] Black Kite
- [ ] SecurityScorecard
- [ ] ServiceNow
- [ ] Jira
- [ ] Slack / Teams
- [ ] Procurement systems

## Advanced

- [ ] Fourth-party graph
- [ ] AI vendor / MCP assessment
- [ ] Risk acceptance workflow
- [ ] Agentic workflow orchestration

