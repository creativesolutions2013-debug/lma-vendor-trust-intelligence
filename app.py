...
def clear_soc2_extraction_state():
    """
    Clear temporary extraction state without
    affecting persisted evidence records.
    """
...
def initialize_evidence_form(
    ...
    """
    Initialize a clean evidence registration form
    whenever the uploaded file changes.
    """

    current_form_hash = st.session_state.get(
...
def populate_evidence_form_from_extraction(
    ...
    """
    Populate the registration form from the
    extraction belonging to the current document.
    """
...
def render_evidence():
    ...
    st.caption(
        ...
        "coverage, provenance, and explainable "
        "evidence analysis."
    )
    ...
    # -----------------------------------------------------
    # Load extraction only for current file
    # -----------------------------------------------------
    ...
    if extraction:
        ...
        st.caption(
            "Analyzed: "
            f"{format_analysis_time(analysis_time)}"
        )
    ...
    with st.form(
        ...
        if st.form_submit_button(
            ...
            if not final_name:
                ...
                if extraction:
                    message += (
                        " Extraction provenance and "
                        "confidence rationale preserved."
                    )
